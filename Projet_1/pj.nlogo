...
globals [
  domaine ;Domaine de valeurs à parcourir pour maximiser la réponse
]

; Limiter les nombres d'agents
...
agents-own [
  ...
  neffort ;moyenne efforts voisins
  nprofit ;moyenne profits voisins
...
to game
  ...
  if antagonist != nobody [
    ...
    let aeff ([effort] of antagonist)

    set profit (func-profit effort aeff)
    set aeffort aeff * bruiter

    set cumprofit (cumprofit + profit)

    ask antagonist [set aprofit profit]
...
to update-agent
  setxy (xcor + max-pxcor / 2 + 1) ycor
  let red-netlogo extract-rgb red
  ; netLogo blue is different than [ 0 0 255 ] it is [52 93 169]
  let blue-netlogo extract-rgb blue
  set color palette:scale-gradient palette:scheme-colors "Divergent" "Spectral" 9 effort 0 2
  ;set color palette:scale-gradient [[0 0 255][0 85 255][0 170 255][0 255 255][0 255 170][0 255 85][0 255 0][85 255 0][170 255 0][255 255 0][255 170 0][255 85 0][255 0 0]] effort 0 2
end
...
to-report best-reply
  ...
  ;foreach x-domain
  ;[
  ;  set profitTmp fct ? aeffort
  ;  set profit-values lput profitTmp profit-values   ; on remplit la liste des profits
  ;]
  ;print profit-values
  ;let positionOfProfitMax position (max profit-values) profit-values  ; on récupère l'indice du profit max
  ;report item positionOfProfitMax x-domain  ; on récupère l'effort qui a donné le profit max
  report 0
end
...
to-report best-reply-average
  ; Pareil que best reply mais sur cumeffort/numinc
  ;let profitTmp 0   ; profit
  ;let profit-values []  ; liste de tous les profits calculés
  ;foreach x-domain
  ;[
